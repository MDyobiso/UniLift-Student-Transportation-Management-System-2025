...
export async function GET(
  ...
  try {
    ...
    const [activeRequestsResult] = await connection.execute(
      'SELECT COUNT(*) as activeRequests FROM request WHERE stud_id = ? AND RequestStatus = ?',
      [stud_id, 'Pending']
    );
    const activeRequests = (activeRequestsResult as any)[0]?.activeRequests || 0;

    // Average Rating (skip for now)
    const averageRating = 0;
    /*
    const [averageRatingResult] = await connection.execute(
      'SELECT AVG(rating) as averageRating FROM ratings WHERE stud_id = ?',
      [stud_id]
    );
